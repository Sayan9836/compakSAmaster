...
@Component({
  ...
  templateUrl: './active-members.page.html',
  styleUrls: ['./active-members.page.scss'],
})
export class ActiveMembersPage implements OnInit {
  ...
  fetchUsers() {
    ...
    this.users = [];
    this.usersService.fetchUsers(1, this.entriesPP, this.currentPage, this.searchTerm);
  }
  makeInactive(userID) {
    this.notice.presentAlertConfirm('You have chosen to make this member inactive.', () => { this.deactivate(userID); });
  }
  confirmRemove(userID) {
    this.notice.presentAlertConfirm('You have chosen to permanently delete this member.', () => { this.remove(userID); });
  }
  remove(userID) {
    this.http.delete(this.domain + '/api/v1/members/' + userID).subscribe(
      (resp: any) => {
