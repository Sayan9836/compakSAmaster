...
export class NoticesService {
  ...
  parseError(error, duplicateText) {
    if (error.error.code === 'E2') {
      this.presentToast(duplicateText, 'danger');
    } else if (error.error.code === 'E100') {
      this.presentToast(error.error.message, 'danger');
    } else {
      console.log(error);
      this.presentToast('A critical error occured.', 'danger');
    }
  }
  loadSub;
  killSub;
